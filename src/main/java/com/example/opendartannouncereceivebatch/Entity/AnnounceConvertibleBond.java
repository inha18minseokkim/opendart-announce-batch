...
@ToString
@Getter
public class AnnounceConvertibleBond implements EssentialReport{
    @Id
    ...
    private Double cvRt;	//전환에 관한 사항(전환비율 (%))
    ...
    private Double cvisstkTisstkVs;	//전환에 관한 사항(전환에 따라 발행할 주식(주식총수 대비 비율(%)))
    private LocalDate cvrqpdBgd;	//전환에 관한 사항(전환청구기간(시작일))
    private LocalDate cvrqpdEdd;	//전환에 관한 사항(전환청구기간(종료일))
    private Long actMktprcflCvprcLwtrsprc;	//전환에 관한 사항(시가하락에 따른 전환가액 조정(최저 조정가액 (원)))
    @Column(length=1000)
    private String actMktprcflCvprcLwtrsprcBs;	//전환에 관한 사항(시가하락에 따른 전환가액 조정(최저 조정가액 근거))
    private Long rmislmtLt70p;	//전환에 관한 사항(시가하락에 따른 전환가액 조정(발행당시 전환가액의 70% 미만으로 조정가능한 잔여 발행한도 (원)))
    @Column(length=1000)
